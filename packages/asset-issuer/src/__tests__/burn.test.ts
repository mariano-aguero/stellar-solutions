...
import { InvalidAddressError, InvalidAmountError, InvalidAssetCodeError, InvalidSecretKeyError } from '@stellar-solutions/core';
...
describe('burn', () => {
  ...
  it('submits payment from distributor back to issuer', async () => {
    const submitMock = vi.fn().mockResolvedValue({ hash: 'burn-tx', ledger: 1, fee_charged: '100' });
    const client = {
